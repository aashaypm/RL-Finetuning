...
def load_model(conf_path):
    ...
    model_path = '../experiments/train/models/10000_G.pth'
    print(model_path)
...
def main():
    ...
    psnr_2 = np.mean(df['PSNR'])
    lrc_psnr = np.mean(df['LRC PSNR'])
    lpips = np.mean(df['LPIPS'])
    str_out = f'SSIM: {ssim}, LPIPS: {lpips}, PSNR: {psnr_2}, LRC PSNR: {lrc_psnr}'
    print(str_out)
