...
class SRFlowModel(BaseModel):
    def __init__(self, opt, step):
        ...
        self.measure = Measure(use_gpu=True)
    ...
    def optimize_parameters(self, step):
        ...
        if weight_fl > 0:
            ...
            for i in range(len(sr)):
                sr_im = 2*(sr[i][None] - 0.5)
                real_H =  2*(self.real_H[i][None] - 0.5)
                score =  self.measure.model.forward(real_H, sr_im).item()
                #score,diff = ssim(sr_im,real_H,full=True, multichannel=True,channel_axis=0,data_range=1)
                scores.append(score)


            
            
            scores = torch.tensor(scores).to(nll.device)
            #scores = torch.clip(scores,min=1e-3,max=1)
            scores = torch.exp(-scores*100)
             
            update_nll = nll*scores
            

            if not torch.any(torch.isnan(update_nll)):
                nll = update_nll
